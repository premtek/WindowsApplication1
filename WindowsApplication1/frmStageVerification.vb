...
Public Class frmStageVerification
    ...
    Private Sub frmStageAccuracy_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        With gSSystemParameter.Pos.StageVerification(sys.StageNo)
            ...
            lblPitchHX.Text = .PitchHX
            lblPitchHY.Text = .PitchHY
            lblPitchVX.Text = .PitchVX
            lblPitchVY.Text = .PitchVY
    ...
    Private Sub frmStageVerification_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub frmStageVerification_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        UcDisplay1.EndLive()
    ...
    Function AnalysisPitchResult() As Boolean
        ...
        Debug.WriteLine("X AccuracyUnit: " & (CDec(nmcStartPosX.Value) + CDec(nmcArrayXCount.Value - 1) * Val(lblPitchHX.Text)) - CDec(nmcCornerPosX.Value))
        Debug.WriteLine("Y AccuracyUnit: " & (CDec(nmcCornerPosY.Value) + CDec(nmcArrayYCount.Value - 1) * Val(lblPitchVY.Text)) - CDec(nmcEndPosY.Value))
        If Math.Abs((CDec(nmcStartPosX.Value) + CDec(nmcArrayXCount.Value - 1) * Val(lblPitchHX.Text)) - CDec(nmcCornerPosX.Value)) > AccuracyUnit Then
            Return False
        End If
        If Math.Abs((CDec(nmcCornerPosY.Value) + CDec(nmcArrayYCount.Value - 1) * Val(lblPitchVY.Text)) - CDec(nmcEndPosY.Value)) > AccuracyUnit Then
            Return False
        End If
        Return True
    End Function
    ...
    Private Sub btnOK_Click(sender As Object, e As EventArgs) Handles btnOK.Click
        ...
        With gSSystemParameter.Pos.StageVerification(sys.StageNo)
            ...
            .PitchHX = CDec(lblPitchHX.Text)
            .PitchHY = CDec(lblPitchHY.Text)
            .PitchVX = CDec(lblPitchVX.Text)
            .PitchVY = CDec(lblPitchVY.Text)
    ...
    Private Async Sub btnGoCCDAlignPos_Click(sender As Object, e As EventArgs) Handles btnGoCCDAlignPos.Click 'Soni / 2017.05.16 去除DoEvents
        ...
        If (lblPitchHX.Text = "0" And lblPitchHY.Text = "0") Or (lblPitchVX.Text = "0" And lblPitchVY.Text = "0") Or lblAngle.Text = "0" Then '先教導
            '請先教導場景
            gSyslog.Save(gMsgHandler.GetMessage(Warn_3000027))
            MsgBox(gMsgHandler.GetMessage(Warn_3000027), MsgBoxStyle.SystemModal + MsgBoxStyle.MsgBoxSetForeground)
            Exit Sub
        ...
        Await Task.Run(Sub()
                           ...
                           TargetPos(0) = nmcStartPosX.Value + intXf * CDec(lblPitchHX.Text) + intYf * CDec(lblPitchVX.Text)
                           TargetPos(1) = nmcStartPosY.Value + intXf * CDec(lblPitchHY.Text) + intYf * CDec(lblPitchVY.Text)
    ...
    Function StageVerification_HorizontalAction() As enmSVTmassage
        ...
        sw.WriteLine("[Pitch], HX:," & lblPitchHX.Text & ", HY:," & lblPitchHY.Text & ", VX:," & lblPitchVX.Text & ", VY:," & lblPitchVY.Text)
        sw.WriteLine("[DelayTime], " & NumSteadyTime.Value)
        '___視覺座標


        '開檔for 軸卡座標
        Dim MotionPath As String = "D:\PIIData\Stage" & (sys.StageNo + 1).ToString & "  Horizontal Test.csv"
        If File.Exists(MotionPath) Then
            File.Delete(MotionPath)
        End If
        Dim MotionSW As StreamWriter = New StreamWriter(MotionPath)
        MotionSW.WriteLine("[STAGE Verification INFORMATION]")
        MotionSW.WriteLine("[Start Pos], X:," & nmcStartPosX.Value & ", Y:," & nmcStartPosY.Value & ", Z:," & nmcStartPosZ.Value)
        MotionSW.WriteLine("[Pitch], HX:," & lblPitchHX.Text & ", HY:," & lblPitchHY.Text & ", VX:," & lblPitchVX.Text & ", VY:," & lblPitchVY.Text)
        MotionSW.WriteLine("[DelayTime], " & NumSteadyTime.Value)
        '___軸卡座標
        ...
        For j As Integer = 0 To nmcArrayYCount.Value - 1
            For i As Integer = 0 To nmcArrayXCount.Value - 1
                ...
                '====計算位置
                targetPosX = nmcStartPosX.Value + i * CDec(lblPitchHX.Text) + j * CDec(lblPitchVX.Text)
                targetPosY = nmcStartPosY.Value + i * CDec(lblPitchHY.Text) + j * CDec(lblPitchVY.Text)
                targetPosZ = nmcStartPosZ.Value
                ...
                If gCCDAlignResultDict(sys.CCDNo)(ticket).Result.Count = 0 Then '結果必須存在
                    ...
                    MotionSW.WriteLine("X:" & "," & i & "," & "Y:" & "," & j & "," & "OffsetmmX:" & "," & "x" & "," & "OffsetmmY:" & "," & "x")
                ElseIf gCCDAlignResultDict(sys.CCDNo)(ticket).Result.Count > 1 Then '畫面有多個特徵
                ...
                Else '有一個特徵
                    ...
                    MotionSW.WriteLine("X:" & "," & i & "," & "Y:" & "," & j & ", OffsetmmX1:," & OffsetmmX.ToString() & ", OffsetmmY1:," & OffsetmmY.ToString() & ", OffsetmmX2:," & OffsetmmX_1.ToString() & ", OffsetmmY2:," & OffsetmmY_1.ToString() & ", DismmX:," & Item_mmX.ToString() & ", DismmY:," & Item_mmY.ToString())
    ...
    Private Async Sub BtnHorizontalRun_Click(sender As Object, e As EventArgs) Handles BtnHorizontalRun.Click 'Soni / 2017.05.16 去除DoEvents
        ...
        If (lblPitchHX.Text = "0") Or (lblPitchVY.Text = "0") Then '先教導
            '請先教導場景
            gSyslog.Save(gMsgHandler.GetMessage(Warn_3000027))
            MsgBox(gMsgHandler.GetMessage(Warn_3000027), MsgBoxStyle.SystemModal + MsgBoxStyle.MsgBoxSetForeground)
            Exit Sub
    ...
    Function StageVerification_VerticalAction() As enmSVTmassage
        ...
        '開檔for 軸卡座標
        Dim MotionPath As String = "D:\PIIData\Stage" & (sys.StageNo + 1).ToString & "  Verification Test.csv"
        If File.Exists(MotionPath) Then
            File.Delete(MotionPath)
        End If
        Dim MotionSW As StreamWriter = New StreamWriter(MotionPath)
    ...
    Private Async Sub BtnVerticalRun_Click(sender As Object, e As EventArgs) Handles BtnVerticalRun.Click
        Await Task.Run(Sub()
                           ...
                           If RunStatus <> enmSVTmassage.Done Then
                               gSyslog.Save("[BtnVerticalRun] RunStatus:" & RunStatus, MsgBoxStyle.SystemModal + MsgBoxStyle.MsgBoxSetForeground)
    ...
    '                targetPosY = nmcStartPosY.Value + i * CDec(lblPitchHY.Text) '+ j * CDec(lblPitchVY.Text)
    '            Else    '往x軸反向走
    '                targetPosX = nmcCornerPosX.Value - i * CDec(lblPitchHX.Text) '+ j * CDec(lblPitchVX.Text)
    '                targetPosY = nmcCornerPosY.Value - i * CDec(lblPitchHY.Text) '+ j * CDec(lblPitchVY.Text)
    '            End If
    '            targetPosZ = nmcStartPosZ.Value
    '            '[說明]:位置安全保護
    '            If CheckGoPos(targetPosX, targetPosY, targetPosZ) = False Then
    ...
    '                        Case 0
    '                            gSyslog.Save(gMsgHandler.GetMessage(Error_1032004))
    '                            MsgBox(gMsgHandler.GetMessage(Error_1032004))
    '                        Case 1
    '                            gSyslog.Save(gMsgHandler.GetMessage(Error_1044004))
    '                            MsgBox(gMsgHandler.GetMessage(Error_1044004))
    '                        Case 2
    '                            gSyslog.Save(gMsgHandler.GetMessage(Error_1062004))
    '                            MsgBox(gMsgHandler.GetMessage(Error_1062004))
    '                        Case 3
    '                            gSyslog.Save(gMsgHandler.GetMessage(Error_1069004))
    '                            MsgBox(gMsgHandler.GetMessage(Error_1069004))
    '                    End Select
    '                    Return enmSVTmassage.AxisMove_TimeOut
    '                    'Exit Function
    '                End If
    ...
    '                           MsgBox("Stage ReRun Vertical Error: " & RunStatus)
    '                       End If
