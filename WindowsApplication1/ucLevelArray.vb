...
Public Class ucLevelArray
    ...
    Public Sub LoadData()
        SetNumericValue(nmcArrayXCount, RecipeEdit.Node(sys.StageNo)(NodeID).Array(mLevelNo).Array.CountX)
        SetNumericValue(nmcArrayXPitch, RecipeEdit.Node(sys.StageNo)(NodeID).Array(mLevelNo).Array.PitchX)
        SetNumericValue(nmcArrayYCount, RecipeEdit.Node(sys.StageNo)(NodeID).Array(mLevelNo).Array.CountY)
        SetNumericValue(nmcArrayYPitch, RecipeEdit.Node(sys.StageNo)(NodeID).Array(mLevelNo).Array.PitchY)
        SetNumericValue(nmcStartPosX, RecipeEdit.Node(sys.StageNo)(NodeID).Array(mLevelNo).Array.StartPosX)
        SetNumericValue(nmcStartPosY, RecipeEdit.Node(sys.StageNo)(NodeID).Array(mLevelNo).Array.StartPosY)
        SetNumericValue(nmcStartPosZ, RecipeEdit.Node(sys.StageNo)(NodeID).Array(mLevelNo).Array.StartPosZ) 'Soni + 2017.01.10 多層陣列
        SetNumericValue(nmcEndPosX, RecipeEdit.Node(sys.StageNo)(NodeID).Array(mLevelNo).Array.EndPosX)
        SetNumericValue(nmcEndPosY, RecipeEdit.Node(sys.StageNo)(NodeID).Array(mLevelNo).Array.EndPosY)
    End Sub
    ...
    Sub UpdateArray(sender As Object, e As EventArgs)
        ...
        If StageNo >= RecipeEdit.Node.Count Then
            '無相應站號
        ...
        If Not RecipeEdit.Node(StageNo).ContainsKey(NodeID) Then
            '無相應節點
        ...
        If LevelNo >= RecipeEdit.Node(StageNo)(NodeID).Array.Count Then
            '該層資料不存在
        ...
        RecipeEdit.Node(StageNo)(NodeID).Array(LevelNo).Array = mArray
        RecipeEdit.Node(StageNo)(NodeID).Array(LevelNo).LevelType = eLevelType.Array
