...
namespace Premtek
{
    /// <summary>直線限制條件
    /// </summary>
    public static class CRecipeStepLineLimit
    {
        /// <summary>弧至少3點
        /// </summary>
        private static int _Dotmin = 2;
        /// <summary>最多不限
        /// </summary>
        ...
        /// <returns></returns>
        public static ErrorCode GetDotLimit(decimal length, decimal acc, decimal cycleTime, out int dotmin, out int dotmax)
        {
            decimal _dotmin1 = 0;
            ...
            if (cycleTime > 0)
            {
                ...
                decimal _pitchmax1 = _velmax * cycleTime;
        ...
        /// <returns></returns>
        public static ErrorCode GetVelLimit(decimal length, decimal acc, decimal cycleTime, out decimal velmin, out decimal velmax)
        {
            decimal _velmin1 = 0;
        ...
        /// <returns></returns>
        public static ErrorCode GetWeightLimit(decimal length, decimal acc, decimal cycleTime, decimal avgWeight, out decimal weightmin, out decimal weightmax)
        {
            if (avgWeight <= 0)
            {
                weightmin = 0;
            ...
            int dotmin = 0;
            int dotmax = 0;
            if (GetDotLimit(length, acc, cycleTime, out dotmin, out dotmax) != ErrorCode.Success)
            {
                weightmin = 0;
                weightmax = 0;
                return ErrorCode.Failed;
