...
namespace Premtek
{
    public partial class ucRecipeLineMid : UserControl
    {
  /// <summary>外部引入系統參數
        /// </summary>
        ...
        public ErrorCode SetValue(CRecipeStep data, CRecipePattern parent, Form parentForm)
        {
            ...
            if (ControlMisc.SetNumericValue(ref this.nmuPosX, this._StepEdit.LineMid.Pos.X) != ErrorCode.Success)
            {
                this._StepEdit.IsCorrect = false;
            }
            if (ControlMisc.SetNumericValue(ref this.nmuPosY, this._StepEdit.LineMid.Pos.Y) != ErrorCode.Success)
            {
                this._StepEdit.IsCorrect = false;
            }
            if (ControlMisc.SetNumericValue(ref this.nmuPosZ, this._StepEdit.LineMid.Pos.Z) != ErrorCode.Success)
            {
                this._StepEdit.IsCorrect = false;
            }
        ...
        public ucRecipeLineMid(ProjectCore.MSystemParameter.sSysParam sys)
        {
            this._IsLoaded = false;
            this._sys = sys;
            InitializeComponent();
