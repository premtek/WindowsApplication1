...
namespace Premtek
{
    public partial class ucRecipeRectangle : UserControl
    {
        /// <summary>外部引入系統參數
        /// </summary>
        ...
        public ErrorCode SetValue(CRecipeStep data, CRecipePattern parent, Form parentForm)
        {
            ...
            if (ControlMisc.SetNumericValue(ref nmuStartX, this._StepEdit.Rectangle.Start.X) != ErrorCode.Success)
            {
                this._StepEdit.IsCorrect = false;
            }
            if (ControlMisc.SetNumericValue(ref nmuStartY, this._StepEdit.Rectangle.Start.Y) != ErrorCode.Success)
            {
                this._StepEdit.IsCorrect = false;
            }
            if (ControlMisc.SetNumericValue(ref nmuStartZ, this._StepEdit.Rectangle.Start.Z) != ErrorCode.Success)
            {
                this._StepEdit.IsCorrect = false;
            }
            if (ControlMisc.SetNumericValue(ref nmuEndX, this._StepEdit.Rectangle.End.X) != ErrorCode.Success)
            {
                this._StepEdit.IsCorrect = false;
            }
            if (ControlMisc.SetNumericValue(ref nmuEndY, this._StepEdit.Rectangle.End.Y) != ErrorCode.Success)
            {
                this._StepEdit.IsCorrect = false;
            }
            if (ControlMisc.SetNumericValue(ref nmuEndZ, this._StepEdit.Rectangle.End.Z) != ErrorCode.Success)
            {
                this._StepEdit.IsCorrect = false;
            }
            if (ControlMisc.SetNumericValue(ref nmuAngle, this._StepEdit.Rectangle.Angle) != ErrorCode.Success)
            {
                this._StepEdit.IsCorrect = false;
            }
        ...
        public ucRecipeRectangle(ProjectCore.MSystemParameter.sSysParam sys)
        {
            this._IsLoaded = false;
            this._sys = sys;
            InitializeComponent();
        ...
        {
            if (!_IsLoaded) return;//載入前不能引發ValueChanged等事件
            this._StepEdit.Rectangle.Start.X = nmuStartX.Value;
            RefreshUI();
        }
        ...
        {
            if (!_IsLoaded) return;//載入前不能引發ValueChanged等事件
            this._StepEdit.Rectangle.Start.Y = nmuStartY.Value;
            RefreshUI();
        }
        ...
        {
            if (!_IsLoaded) return;//載入前不能引發ValueChanged等事件
            this._StepEdit.Rectangle.Start.Z = nmuStartZ.Value;
            RefreshUI();
        }
        ...
        {
            if (!_IsLoaded) return;//載入前不能引發ValueChanged等事件
            this._StepEdit.Rectangle.End.X = nmuEndX.Value;
            RefreshUI();
        }
        ...
        {
            if (!_IsLoaded) return;//載入前不能引發ValueChanged等事件
            this._StepEdit.Rectangle.End.Y = nmuEndY.Value;
            RefreshUI();
        }
        ...
        {
            if (!_IsLoaded) return;//載入前不能引發ValueChanged等事件
            this._StepEdit.Rectangle.End.Z = nmuEndZ.Value;
            RefreshUI();
        }
        ...
        {
            if (!_IsLoaded) return;//載入前不能引發ValueChanged等事件
            this._StepEdit.Rectangle.Angle = nmuAngle.Value;
        }
