...
namespace Premtek
{
    public partial class ucRecipeTimesUp : UserControl
    {
    /// <summary>外部引入系統參數
        /// </summary>
        ...
        public ErrorCode SetValue(CRecipeStep data, CRecipePattern parent, Form parentForm)
        {
            ...
            nmuDelay.Value = this._StepEdit.TimesUp.DelayTimeInSec;
            if ((this._StepEdit.TimesUp.Name != null) && (cmbTimer.Items.Contains(this._StepEdit.TimesUp.Name)))
            {
                cmbTimer.SelectedItem = this._StepEdit.TimesUp.Name;
        ...
        public ucRecipeTimesUp(ProjectCore.MSystemParameter.sSysParam sys)
        {
            this._IsLoaded = false;
            this._sys = sys;
            InitializeComponent();
        ...
        private void cmbTimer_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                cmbTimer.BackColor = SystemColors.Window;
                this._StepEdit.TimesUp.Name = cmbTimer.SelectedItem.ToString();
            }
        }
        ...
        {
            if (!_IsLoaded) return;//載入前不能引發ValueChanged等事件
            this._StepEdit.TimesUp.DelayTimeInSec = nmuDelay.Value;
