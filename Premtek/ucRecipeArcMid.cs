...
namespace Premtek
{
    public partial class ucRecipeArcMid : UserControl
    {
        /// <summary>外部引入系統參數
        /// </summary>
        ...
        public ErrorCode SetValue(CRecipeStep data, CRecipePattern parent, Form parentForm)
        {
            ...
            if (ControlMisc.SetNumericValue(ref this.nmuPosX, this._StepEdit.ArcMid.Pos.X) != ErrorCode.Success)
            {
                this._StepEdit.IsCorrect = false;
            }
            if (ControlMisc.SetNumericValue(ref this.nmuPosY, this._StepEdit.ArcMid.Pos.Y) != ErrorCode.Success)
            {
                this._StepEdit.IsCorrect = false;
            }
            if (ControlMisc.SetNumericValue(ref this.nmuPosZ, this._StepEdit.ArcMid.Pos.Z) != ErrorCode.Success)
            {
                this._StepEdit.IsCorrect = false;
            }
        ...
        public ucRecipeArcMid(ProjectCore.MSystemParameter.sSysParam sys)
        {
            this._IsLoaded = false;
            this._sys = sys;
            InitializeComponent();
        ...
        private void nmuPosX_ValueChanged(object sender, EventArgs e)
        {
            ...
            this._StepEdit.ArcMid.Pos.X = nmuPosX.Value;
        }

        private void nmuPosY_ValueChanged(object sender, EventArgs e)
        {
            if (!_IsLoaded) return;//載入前不能引發ValueChanged等事件
            this._StepEdit.ArcMid.Pos.Y = nmuPosY.Value;
        }

        private void nmuPosZ_ValueChanged(object sender, EventArgs e)
        {
            if (!_IsLoaded) return;//載入前不能引發ValueChanged等事件
            this._StepEdit.ArcMid.Pos.Z = nmuPosZ.Value;
        }
